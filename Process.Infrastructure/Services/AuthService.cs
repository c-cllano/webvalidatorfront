...
namespace Process.Infrastructure.Services
{
    public class AuthService : Domain.Services.IAuthService
    {
        ...
        private readonly string _certificate_Path;
        private readonly string _certificate_Password;
        ...
        public AuthService(IHttpClientFactory clientFactory, IConfiguration config)
        {
            ...
            _certificate_Path = _config.GetSection("certificate_Path")?.Value ?? string.Empty;
            _certificate_Password = _config.GetSection("certificate_Password")?.Value ?? string.Empty;
        }
        ...
        public async Task<string> GetNewToken(string accessToken, Guid procesoConvenioGuid)
        {
            ...
            X509SigningCredentials clientSigningCredentials = new(GetCertificadoSigningCredentials.GetCertificateInfo(_certificate_Path, _certificate_Password));
