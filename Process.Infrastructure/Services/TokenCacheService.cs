...
namespace Process.Infrastructure.Services
{
    ...
    ) : ITokenCacheService
    {
        ...
        public async Task SetTokenCacheAsync<T>(string microserviceKey, T token, long? expiresInSeconds = null)
        {
            expiresInSeconds ??= long.Parse(_config["TokenCacheExpiredSeconds"]!);
