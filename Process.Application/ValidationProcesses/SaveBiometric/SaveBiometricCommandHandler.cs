...
{
    public class SaveBiometricCommandHandler(
        IValidationProcessRepository validationProcessRepository,
        IAgreementOkeyStudioRepository agreementOkeyStudioRepository,
        IPlatformConnectionStrategyService<ISaveBiometricService> platformStrategy,
        ReconoserContext context
    ) : IRequestHandler<SaveBiometricCommand, SaveBiometricResponse>
    {
        ...
        private readonly IPlatformConnectionStrategyService<ISaveBiometricService> _platformStrategy = platformStrategy;
        private readonly ReconoserContext _context = context;

        public async Task<SaveBiometricResponse> Handle(SaveBiometricCommand request, CancellationToken cancellationToken)
        {
            ...
            return await strategy.SaveBiometricAsync(request);
        }
    }
}
