...
{
    public class ValidateBiometricCommandHandler(
        IValidationProcessRepository validationProcessRepository,
        IAgreementOkeyStudioRepository agreementOkeyStudioRepository,
        IPlatformConnectionStrategyService<IValidateBiometricService> platformStrategy,
        ReconoserContext context
    ) : IRequestHandler<ValidateBiometricCommand, ValidateBiometricResponse>
    {
        ...
        private readonly IPlatformConnectionStrategyService<IValidateBiometricService> _platformStrategy = platformStrategy;
        private readonly ReconoserContext _context = context;

        public async Task<ValidateBiometricResponse> Handle(ValidateBiometricCommand request, CancellationToken cancellationToken)
        {
            ...
            return await strategy.ValidateBiometricAsync(request, validationProcess);
        }
    }
}
